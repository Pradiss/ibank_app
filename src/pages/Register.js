...
export default function Register({ navigation }) {
    ...
    const Register = async () => {
        ...
        try {
            ...
            console.log("dentro do send", name)
        } catch (error) {
            Alert.alert("Erro ao criar um Usuario", error)
            console.log(error)
        }
        console.log("deposi do catch", name)
    }
            ...
            <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
                <ScrollView contentContainerStyle={{ padding: 16, paddingTop: 70 }}>
                    <View style={{ alignItems: "center" }}>
                        ...
                        <View style={{ flexDirection: "row", gap: 16, paddingHorizontal: 6 }}>
                            <TextInputMask
                                type={'cpf'}
                                value={cpf}
                        ...
                        <View style={{ flexDirection: "row", paddingHorizontal: 6, gap: 16 }}>
                            <TextInput style={[styles.input, { width: "60%" }]} placeholder="Nome da Rua " value={street} onChangeText={setStreet} />
                            <TextInputMask
                                placeholder="CEP"
                                type={'zip-code'}
                                value={cep}
                        ...
                        <View style={{ flexDirection: "row", paddingHorizontal: 6, gap: 16 }}>
                            <TextInput style={[styles.input, { width: "70%" }]} placeholder="Bairro" value={location} onChangeText={setLocation} />
                            <TextInputMask
                                type={"custom"}
                                placeholder="N° "
                                options={{
                        ...
                        <View style={{ flexDirection: "row", paddingHorizontal: 6, gap: 16 }}>
                            <TextInput style={[styles.input, { width: "70%" }]} placeholder="Cidade" value={city} onChangeText={setCity} />
                            <TextInput
                                style={[styles.input, { width: "30%" }]}
                                placeholder="Estado "
                                value={state}
                                onChangeText={(text) => setState(text.replace(/[^a-zA-Z]/g, '').toUpperCase(0, 2))}
                            />
